...
public class BlobTest extends Common {
  ...
  @Test
  public void length() {
    TiDBBlob blob = new TiDBBlob(bytes);
    assertEquals(6, blob.length());

    TiDBBlob blob2 = new TiDBBlob(bytes, 2, 3);
  ...
  public void position() throws SQLException {
    ...
    TiDBBlob blob2 = new TiDBBlob(bytes, 2, 4);
    ...
    assertEquals(3, blob2.position(new TiDBBlob(new byte[] {4, 5}), 2));
  ...
  public void setBytesOffset() throws SQLException {
    ...
    TiDBBlob blob = new TiDBBlob(new byte[] {0, 1, 2, 3, 4, 5});
    ...
    TiDBBlob blob3 = new TiDBBlob(new byte[] {0, 1, 2, 3, 4, 5}, 2, 4);
    ...
    try {
      TiDBBlob blob6 = new TiDBBlob(new byte[] {0, 1, 2, 3, 4, 5}, 2, 3);
  ...
  public void expectedErrors() {
    assertThrowsContains(
        IllegalArgumentException.class, () -> new TiDBBlob(null), "byte array is null");
    assertThrowsContains(
        IllegalArgumentException.class, () -> new TiDBBlob(null, 0, 2), "byte array is null");
  }
  ...
  public void equal() {
    ...
    assertEquals(new TiDBBlob(bytes), blob);
    assertFalse(blob.equals(null));
    assertFalse(blob.equals(""));
    byte[] bytes = new byte[] {5, 1, 2, 3, 4, 5};
    assertNotEquals(new TiDBBlob(bytes), blob);
    assertNotEquals(new TiDBBlob(new byte[] {5, 1}), blob);
  }
